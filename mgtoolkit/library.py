...
class Triple(object):
    ...
    def __hash__(self):
        ...
        # Tolerate None values for coinputs/cooutputs/edges.
        cins = frozenset(self.coinputs) if getattr(self, "coinputs", None) is not None else frozenset()
        couts = frozenset(self.cooutputs) if getattr(self, "cooutputs", None) is not None else frozenset()
        ...
        if isinstance(edges, list):
            ...
            edges_key = (edges,)
        return hash((cins, couts, edges_key))
...
class Edge(object):
    ...
    def label(self):
        ...
        return self.label
    def __hash__(self):
            return hash((frozenset(self.invertex), frozenset(self.outvertex), tuple(self.attributes) if self.attributes else None, self.label))
    def __eq__(self, other):
        if other is None:
            return False
...
class Metagraph(object):
    ...
    def adjacency_matrix(self):
        ...
        return np.array(adj_matrix, dtype=object)
    ...
    def get_closure(self):
        """ Returns the closure np.array(i.e., A*) of the metagraph.
        :return: numpy.ndarray
        """
        ...
        # noinspection PyCallingNonCallable
        return np.array(a_star, dtype=object)
    ...
    def is_input_dominant_metapath(self, metapath):
        ...
        all_subsets = sum([list(combinations(metapath.source, r)) for r in range(1, len(metapath.source)+1)], [])
    ...
    def is_redundant_edge(self, edge, metapath, source, target):
        ...
        from itertools import combinations
        all_subsets = sum([list(combinations(target, r)) for r in range(1, len(target)+1)], [])
    ...
    def get_minimal_cutset(self, source, target):
        ...
        for metapath in metapaths:
            all_combinations = sum([list(combinations(metapath.edge_list, r)) for r in range(1, len(metapath.edge_list)+1)], [])
    ...
    def get_projection(self, generator_subset):
        ...
        from itertools import combinations
        all_combinations = sum([list(combinations(all_triples, r)) for r in range(1, len(all_triples)+1)], [])
        for combination in all_combinations:
    ...
    def incidence_matrix(self):
        ...
        # noinspection PyCallingNonCallable
        return np.array(incidence_matrix, dtype=object)
    ...
    def dominates(self, metagraph2):
        ...
        from itertools import combinations
        all_sources1 = sum([list(combinations(self.generating_set, r)) for r in range(1, len(self.generating_set))], [])
        all_targets1 = copy.copy(all_sources1)

        all_sources2 = sum([list(combinations(metagraph2.generating_set, r)) for r in range(1, len(metagraph2.generating_set))], [])
    ...
    def make_ordered_metapath(self, source, target):
        """ Creates an ordered metapath from the given edges and source.
        ...
        if not isinstance(edges, list):
            raise MetagraphException('edges', resources['format_invalid'])
        for edge in edges:
            if not isinstance(edge, Edge):
                raise MetagraphException('edge', resources['format_invalid'])
        metapath_edges = []
        ...
        while added != False:
            ...
            for edge in edges:
                if edge in metapath_edges:
                    continue
                if edge.invertex.issubset(current_bag):
                    metapath_edges.append(edge)
                    current_bag = current_bag.union(edge.outvertex)
                    # edges.remove(edge)
                    added = True
    ...
    def is_ordered_metapath(self, metapath, is_metapath_handled=False):
        ...
        if metapath is None:
            raise MetagraphException('metapath', resources['value_null'])
        if not isinstance(metapath, Metapath):
            raise MetagraphException('metapath', resources['format_invalid'])
        # check input metapath is valid
        if not self.is_metapath(metapath):
            if is_metapath_handled:
                return False
            raise MetagraphException('metapath', resources['arguments_invalid'])
        #check if source and target in metagraph object
        
        source = metapath.source
        target = metapath.target
        edges = list(metapath.edge_list)
        current_bag = set(source)
        for edge in edges:
            if not edge.invertex.issubset(current_bag):
                return False
            current_bag = current_bag.union(edge.outvertex)

        return target.issubset(current_bag)
    
    def is_edge_dominating_ordered_metapath(self, metapath):
        """ Checks if an edge in the metapath dominates the metapath.
        :param metapath: Metapath object
        :return: boolean
        """
        ...
        if not isinstance(metapath, Metapath):
            raise MetagraphException('metapath', resources['format_invalid'])
        # check if ordered metapath
        if not self.is_ordered_metapath(metapath):
            raise MetagraphException('metapath', resources['arguments_invalid'])
        ...
        edges = list(metapath.edge_list)
        for edge in edges:
            #deep copy edges and remove edge from edges
            tmp_metapath = Metapath(source, target, list(metapath.edge_list))
            tmp_metapath.edge_list.remove(edge)
            # remove edge from metapath 
            if self.is_ordered_metapath(tmp_metapath, is_metapath_handled=True):
                return False
        return True

    def is_input_dominating_ordered_metapath(self, metapath):
        """ Checks if an input in the metapath dominates the metapath.
        :param metapath: Metapath object
        :return: boolean
        """
        if metapath is None:
        ...
        edges = list(metapath.edge_list)
        for input in source:
           tmp_metapath = Metapath(copy.deepcopy(source), target, edges)
           tmp_metapath.source.remove(input)
           if self.is_ordered_metapath(tmp_metapath, is_metapath_handled=True):
               return False
        return True
    
    def make_edge_dominant(self, metapath):
        """ Converts the given metapath to an edge-dominant ordered metapath.
        :param metapath: Metapath object
        :return: Metapath object
        """
        ...
        if not self.is_metapath(metapath):
            raise MetagraphException('metapath', resources['arguments_invalid'])
        if not self.is_ordered_metapath(metapath, is_metapath_handled=True):
            raise MetagraphException('metapath', resources['arguments_invalid'])
        ...
        edges = list(metapath.edge_list)
        changed = True
        tmp_edges = list(edges)
        while changed:
            changed = False
            for edge in edges:
                if edge not in tmp_edges:
                    continue
                tmp_metapath = Metapath(source, target, list(tmp_edges))
                tmp_metapath.edge_list.remove(edge)
                if self.is_ordered_metapath(tmp_metapath, is_metapath_handled=True):
                    changed = True
    ...
    def make_input_dominant(self, metapath):
        """ Converts the given metapath to an input-dominant ordered metapath.
        :param metapath: Metapath object
        :return: Metapath object
        """
        ...
        while changed:
            changed = False
            for input in source:
                if input not in tmp_source:
                    continue
                tmp_metapath = Metapath(copy.deepcopy(tmp_source), target, edges)
                tmp_metapath.source.remove(input)
                if self.is_ordered_metapath(tmp_metapath, is_metapath_handled=True):
                    changed = True
    ...
    def get_dominant_metapath(self, source, target):
        ...
        if target is None or len(target) == 0:
            raise MetagraphException('target', resources['value_null'])
        metapath = self.make_ordered_metapath(source, target)
        if metapath is None:
            return None
        metapath = self.make_edge_dominant(metapath)
        metapath = self.make_input_dominant(metapath)
