...
def get_dominant_morphisms(hc: Hypergraph, src: set[str] | None = None, dest: set[str] | None = None):    
    """
    Find a composition of boxes connection src to dest that is both input-dominant and box-dominant.
    ...
    """
    if src == None or len(src) == 0: src = set([o.name for o in hc.dom])
    if dest == None or len(dest) == 0: dest = set([o.name for o in hc.cod])
